...
def _transform_img_gpu(img, M, dsize, flags=CV2_INTERP, borderMode=None):
    """GPU加速的图像仿射变换
    img: numpy array (H, W, C) or torch tensor
    ...
    if isinstance(img, np.ndarray):
        ...
        img_tensor = img.to(DEVICE) if img.device != DEVICE else img

    # 确保变换矩阵是torch tensor
    if isinstance(M, np.ndarray):
        M_tensor = torch.from_numpy(M[:2, :]).float().to(DEVICE)
    else:
        M_tensor = M[:2, :].to(DEVICE) if M.device != DEVICE else M[:2, :]
    ...
    # 构建仿射变换矩阵 (PyTorch需要3x3矩阵)
    if M_tensor.shape == (2, 3):
        M_3x3 = torch.eye(3, device=DEVICE, dtype=M_tensor.dtype)
        M_3x3[:2, :] = M_tensor
    else:
        M_3x3 = M_tensor

    # 执行仿射变换
    grid = F.affine_grid(M_3x3[:2, :].unsqueeze(0), img_tensor.size(), align_corners=False)
    transformed = F.grid_sample(img_tensor, grid, mode='bilinear', align_corners=False, padding_mode='zeros')

    # 调整输出尺寸
    if transformed.shape[-2:] != _dsize[::-1]:  # PyTorch是(H, W)格式
        transformed = F.interpolate(transformed, size=_dsize[::-1], mode='bilinear', align_corners=False)

    # 转换回numpy格式
    result = transformed.squeeze(0).permute(1, 2, 0).cpu().numpy()
...
def _transform_img(img, M, dsize, flags=CV2_INTERP, borderMode=None):
    ...
    #     return _transform_img_gpu(img, M, dsize, flags, borderMode)
    # else:
    if isinstance(dsize, tuple) or isinstance(dsize, list):
        _dsize = tuple(dsize)
    else:
